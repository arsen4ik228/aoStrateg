...
import { getOrganizationList } from '../../../../BLL/admin/userSlice';
const ListOfAcademy = ({ onSelectedOrganizations }) => {

    const accountId = useParams();
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(getOrganizationList({ accountId: accountId }))
    }, [accountId]);

    const organizationList = useSelector((state) => (state.adminUsers?.organizations || []));
    ...
    return (
        <Grid container sx={{ height: 'calc(100vh - 280px)', color: 'black', borderBottom: '1px solid #B4B4B4', fontFamily: "'Montserrat', sans-serif", top: 210, left: 0, bottom: 0, right: 0, weight: '100%', overflow: 'auto', position: 'fixed' }} >
